@echo off
REM Launch using Local Python (runtime\ from Install.bat, else %%LOCALAPPDATA%%\WallpaperRecolor).
cd /d "%~dp0"
...
if not exist "%PY%" (
  echo Run Install.bat once first ^(needs network; installs Local Python in this folder^).
  pause
  exit /b 1
)
if exist "%PYW%" (
